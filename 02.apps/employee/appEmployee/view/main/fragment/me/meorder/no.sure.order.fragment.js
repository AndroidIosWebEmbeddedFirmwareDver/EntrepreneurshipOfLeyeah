...
export default class NoSureOrderFragment extends PureComponent {
    //1.
    constructor(props) {
        super(props);
        this.state = {
    ...
    render() {
        return (
            <View style={NoSureOrderFragmentStyles.container}>
                {this.state.responValue ?
                    <View style={NoSureOrderFragmentStyles.function_list}>
                        <FlatList
                            ref={(flatList) => this._flatList = flatList}
                            data={this.state.responValue.data}
                            keyExtractor={this._keyExtractor}
                            renderItem={({item, index}) => this._renderFlatListItem(item, index)}
                            contentContainerStyle={NoSureOrderFragmentStyles.function_list_contentContainer}
                        />
                    </View>
                    : <Text
                        style={NoSureOrderFragmentStyles.normal_text}>{this.state.backViewShow}</Text>
                }
            </View>
        );
    };
...
NoSureOrderFragmentStyles = StyleSheet.create({
    //基于flex-box布局
    container: {
        width: width,
        flex: 1,
