...
export default class ResumeChoiceWorkSelfevalsActivity extends RefreshCommonActivity {
    ...
    //查询数据
    _initQueryData = () => {
        NetworkCommonUtil.httpGetRequest(NetworkCommonUtil.SERVERURL_API_MOBILE_USER_RESUME_SELFSVALS_QUERYALL).then((val: any) => {
    ...
    _generNetToLocalData = (val) => {
        if (val && val.data && this.props.mobileUserData && this.props.mobileUserData.data && this.props.mobileUserData.data.resume && this.props.mobileUserData.data.resume.workSelfEvals) {
            for (let i of val.data) {
                for (let work of JSON.parse(this.props.mobileUserData.data.resume.workSelfEvals)) {
                    if (i.id === work.id) {
                        i.selected = true;
                    }
                }
    ...
    _commitData = () => {
        ...
        let params = {
            resume: {
                ...
                workSelfEvals: JSON.stringify(workSelfEvals),
            },
            workSelfEvals: workSelfEvals,
        };
        NetworkCommonUtil.httpPostRequest(JSON.stringify(params), NetworkCommonUtil.SERVERURL_SYS_POST_USER_RESUME_UPDATE_WORKSELFEVALS).then((val: any) => {
            DialogManagerUtil.hide();
            if (val && val.code === NetworkCommonUtil.SERVER_HTTP_TASK_STATUS_SUCCESS) {
                // ToastManager.show(val.msg + '', ToastManager.DURATION.LONG, ToastManager.POSITION.BOTTOM);
                this._onSuccessCallback(params.resume.workSelfEvals);
            } else if (val) {
                ToastManager.show(val.msg + '', ToastManager.DURATION.SHORT, ToastManager.POSITION.BOTTOM);
            } else {
                ToastManager.show('请求网络出错' + '', ToastManager.DURATION.SHORT, ToastManager.POSITION.BOTTOM);
    ...
    //提交成功
    _onSuccessCallback = (workSelfEvals) => {
        UserDataManager.getInstance().saveResumeOfWorkSelfEvals(workSelfEvals).then(saveToMap => {
        ...
        return (
            <View style={PhoneResetActivityStyles.container}>
                <HeaderCommonView navigator={this.props.navigator}
                                  ...
                                  headerCenterTitle={'自我评价'}/>
                <View style={ResumeChoiceWorkSelfevalsActivityStyles.function_list}>
...
ResumeChoiceWorkSelfevalsActivityStyles = StyleSheet.create({
    //基于flex-box布局
    container: {
        flex: 1,
