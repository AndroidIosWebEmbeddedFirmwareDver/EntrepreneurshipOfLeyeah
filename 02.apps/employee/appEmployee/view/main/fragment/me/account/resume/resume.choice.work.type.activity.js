...
export default class ResumeChoiceWorkTypeActivity extends RefreshCommonActivity {
    ...
    _generNetToLocalData = (val) => {
        if (val && val.data && this.props.mobileUserData && this.props.mobileUserData.data && this.props.mobileUserData.data.resume && this.props.mobileUserData.data.resume.workTypes) {
            for (let i of val.data) {
                for (let work of JSON.parse(this.props.mobileUserData.data.resume.workTypes)) {
                    if (i.id === work.id) {
                        i.selected = true;
                    }
                }
    ...
    _commitData = () => {
        ...
        NetworkCommonUtil.httpPostRequest(JSON.stringify(params), NetworkCommonUtil.SERVERURL_SYS_POST_USER_RESUME_UPDATE_WORKTYPES).then((val: any) => {
            DialogManagerUtil.hide();
            if (val && val.code === NetworkCommonUtil.SERVER_HTTP_TASK_STATUS_SUCCESS) {
                // ToastManager.show(val.msg + '', ToastManager.DURATION.LONG, ToastManager.POSITION.BOTTOM);
                this._onSuccessCallback(params.resume.workTypes);
            } else if (val) {
                ToastManager.show(val.msg + '', ToastManager.DURATION.SHORT, ToastManager.POSITION.BOTTOM);
            } else {
                ToastManager.show('请求网络出错' + '', ToastManager.DURATION.SHORT, ToastManager.POSITION.BOTTOM);
    ...
    _onSuccessCallback = (data) => {
        UserDataManager.getInstance().saveResumeOfWorkTypes(data).then(saveToMap => {
    ...
    render() {
        return (
            <View style={PhoneResetActivityStyles.container}>
                ...
                <View style={ResumeChoiceWorkTypeActivityStyles.function_list}>
...
ResumeChoiceWorkTypeActivityStyles = StyleSheet.create({
    //基于flex-box布局
    container: {
        flex: 1,
