...
export default class ResumeChoiceWorkExprenceActivity extends RefreshCommonActivity {
    ...
    _generNetToLocalData = (val) => {
        if (val && val.data) {
            for (let i of val.data) {
                ...
                if (this.props.mobileUserData && this.props.mobileUserData.data && this.props.mobileUserData.data.resume && this.props.mobileUserData.data.resume.workExperienceType) {
                    if (i.id === this.props.mobileUserData.data.resume.workExperienceType) {
                        i.selected = true;
                    }
                }
            }
    ...
    _commitData = () => {
        ...
        if (workTypes.length !== 1) {
            ToastManager.show('请选择一项' + '', ToastManager.DURATION.SHORT, ToastManager.POSITION.BOTTOM);
            return;
        }
        ...
        let params = {
            resume: {
                ...
                workExperienceType: workTypes[0].id,
                workExperienceTypeDesc: workTypes[0].description,
            },
        };
        NetworkCommonUtil.httpPostRequest(JSON.stringify(params), NetworkCommonUtil.SERVERURL_SYS_POST_USER_RESUME_UPDATE_WORKEXPERIENCE).then((val: any) => {
            DialogManagerUtil.hide();
            if (val && val.code === NetworkCommonUtil.SERVER_HTTP_TASK_STATUS_SUCCESS) {
                // ToastManager.show(val.msg + '', ToastManager.DURATION.LONG, ToastManager.POSITION.BOTTOM);
                this._onSuccessCallback(params.resume.workExperienceType, params.resume.workExperienceTypeDesc);
            } else if (val) {
                ToastManager.show(val.msg + '', ToastManager.DURATION.SHORT, ToastManager.POSITION.BOTTOM);
            } else {
                ToastManager.show('请求网络出错' + '', ToastManager.DURATION.SHORT, ToastManager.POSITION.BOTTOM);
    ...
    //提交成功
    _onSuccessCallback = (workExperienceType, workExperienceTypeDesc) => {
        UserDataManager.getInstance().saveResumeOfWorkExperience(workExperienceType, workExperienceTypeDesc).then(saveToMap => {
        ...
        return (
            <View style={PhoneResetActivityStyles.container}>
                <HeaderCommonView navigator={this.props.navigator}
                                  ...
                                  headerCenterTitle={'我的工种'}/>
                <View style={ResumeChoiceWorkExprenceActivityStyles.function_list}>
...
ResumeChoiceWorkExprenceActivityStyles = StyleSheet.create({
    //基于flex-box布局
    container: {
        flex: 1,
