...
export default class CallPhoneActivity extends BaseActivity {
    //1.
    constructor(props) {
        super(props);
        this.state = {
    ...
    _uploadName = () => {
        ...
        let params = {
            ...
            callPhone: this.state.nameText,
            rnaOrder: {
                userId: this.props.mobileUserData.data.id,
                userCallPhoneNo: this.state.nameText,
            }
        };
        NetworkCommonUtil.httpPostRequest(JSON.stringify(params), NetworkCommonUtil.SERVERURL_SYS_USER_EMPLOYER_UPDATE_CALLPHONE).then((val: any) => {
            ...
            if (val && val.code === NetworkCommonUtil.SERVER_HTTP_TASK_STATUS_SUCCESS) {
                ...
                if (val.data) {
                    this._onNameChangeSuccessCallback(params.callPhone);
                }
            } else if (val) {
                ToastManager.show(val.msg + '', ToastManager.DURATION.SHORT, ToastManager.POSITION.BOTTOM);
            } else {
    ...
    _onNameChangeSuccessCallback = (callPhoneNo) => {
        UserDataManager.getInstance().saveCallPhoneNo(callPhoneNo).then(saveToMap => {
    ...
    render() {
        return (
            <View style={CallPhoneActivityStyles.container}>
                <HeaderCommonView navigator={this.props.navigator}
                                  ...
                                  headerCenterTitle={'联系电话'}/>
                <View style={CallPhoneActivityStyles.function_list}>
                    <View style={{height: 10, backgroundColor: 'transparent',}}/>
                    <View style={[CallPhoneActivityStyles.function_list_item, {}]}>
                        <TextInput style={[CallPhoneActivityStyles.function_list_item_view, {
                            margin: 9.6,
                            padding: 0,
                        }]}
                                   underlineColorAndroid="transparent"
                                   placeholder={
                                       this.props.mobileUserData && this.props.mobileUserData.data &&
                                       this.props.mobileUserData.data.callPhone ? this.props.mobileUserData.data.callPhone : ''}
...
CallPhoneActivityStyles = StyleSheet.create({
    //基于flex-box布局
    container: {
        flex: 1,
