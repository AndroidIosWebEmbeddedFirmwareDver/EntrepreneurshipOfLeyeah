    ...
    _uploadAvater = (imageNetUrl) => {
        ...
        NetworkCommonUtil.httpPostRequest(JSON.stringify(params), NetworkCommonUtil.SERVERURL_SYS_USER_UPDATE_AVATER).then((val: any) => {
            ...
            if (val && val.code === NetworkCommonUtil.SERVER_HTTP_TASK_STATUS_SUCCESS) {
                ...
                if (val.data) {
                    this.setState({
                        imageCacheUrl: val.data.avatarUrl,
                    });
                    this._onAvatorChangeSuccessCallback(val.data.avatarUrl);
                }
            } else if (val) {
                ToastManager.show(val.msg + '', ToastManager.DURATION.SHORT, ToastManager.POSITION.BOTTOM);
            } else {
                                ...
                                <Image
                                    source={
                                        this.state.imageCacheUrl ? {uri: this.state.imageCacheUrl}
                                            : this.props.mobileUserData && this.props.mobileUserData.data && this.props.mobileUserData.data.avatarUrl ? {uri: this.props.mobileUserData.data.avatarUrl} : require('../../../../../stable/image/app_a.png')}
                                    style={{
                                        width: width,
                                        height: 240,
                                        resizeMode: 'cover',
